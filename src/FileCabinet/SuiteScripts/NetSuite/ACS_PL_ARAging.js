...
define(['N/log', 'N/search'],
    function (log, search) {
        const renderPortlet = (params) => {
            ...
            let customerSearch = search.load({id: 'customsearch2627'});
            let topTen = customerSearch.run().getRange({start: 0, end: 10});
            topTen.forEach(result => {
                let row = {};
                row.customer = result.getValue(result.columns[0])
            ...
            let rowOther = {};
            rowOther.customer = '<a class="dottedlink viewitem" href="https://7336086.app.netsuite.com/app/common/search/searchresults.nl?searchid=2627&saverun=T&whence=">Other</a>';
